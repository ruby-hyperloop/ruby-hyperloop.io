...
<div class="buttons-unit downloads">
  <a href="/chatrb.html" class="button">
    ChatRB Demo App
  </a>
</div>
...
- #### HTML tags are Ruby methods
    ...
    All HTML tags have lower case names matching their HTML counter parts, with the exception of the `p` tag, which has is generated by the `para` method.
...
class Nav < React::Component::Base
  ...
  def valid_new_input?
    state.user_name_input != state.current_user_name &&
    state.user_name_input.strip != ""
  end
...
As each new set of messages arrives the block will execute which will initialize or append the messages to the `state.messages` state variable.  We know we want to make this an state variable since it clearly is going to change asynchronously overtime, and we will want to update the UI when that occurs.
...
- #### Not everything has to be a state variable
    ...
    We can see that as time passes new messages will come in, and we will want to re-render when this happens.  This led us to define `messages` as state variable.  Because `messages` is a state variable as it changes re-rendering of the App component will automatically occur.
...
For our simple App we are going to figure that we are logged in **if** we `state.messages` is not `nil`.  
...
You may be noticed that when you enter a message, you can't see it until you scroll down.  We would like the automatically scroll down when a message is sent so you can see it.  While we are at it, we will also make any new incoming messages do the same thing.
...
In the `Nav` component you will need to add two event handlers, one to track the `key_down` event, and the other to clear the `submit` event for the form.
